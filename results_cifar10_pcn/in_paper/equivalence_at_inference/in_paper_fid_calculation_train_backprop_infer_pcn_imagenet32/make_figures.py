#!/usr/bin/env python3
"""Paper sample grids for the ImageNet-32 train-backprop / infer-PCN section.

Inputs (from submit_scripts/in_paper/in_paper_submit_imagenet32_sample_grid.sh,
which replays the seeded first batch of FID jobs 34743305/34743306):
    samples_main641k_tau2.5_seed1_{backprop,pcn}.npy   uint8, 32 x 3 x 32 x 32

Outputs:
    figure_E_imagenet32_pcn_samples.png        PCN relaxation only
    figure_F_imagenet32_ffn_vs_pcn_samples.png two stacked blocks, matched seeds
...
def load(tag, n):
    p = os.path.join(HERE, f"samples_main641k_tau2.5_seed1_{tag}.npy")
...
grid([load("pcn", N)], "figure_E_imagenet32_pcn_samples.png", A.rows)
...
grid([ffn, pcn], "figure_F_imagenet32_ffn_vs_pcn_samples.png", A.rows_f,
...
# difference is amplified by 250 SDE steps of a chaotic sampler. Quantify both
