...
device = 'cuda'
CKPTS = [
    ('/workspace/EnergyMatchingEqProp/results_cifar10_pcn/stage_3_ep/CHANGE_ME/checkpoint_5000.pt', 'step5k'),
]
...
ut = torch.randn(64, 3, 32, 32, device=device)
...
for ckpt_path, label in CKPTS:
    ...
    for beta_val in [0.1, 0.05, 0.01]:
        m = PCNVelocityWrapper(T_free=10, error_param=True, param_grad_mode='ep',
            lambda_spring=1.0, beta=beta_val, K_h=1, output_scale=1000.0).to(device)
