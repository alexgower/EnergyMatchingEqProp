...
# Energy Matching training for CIFAR-10.
# Supports Phase 1 (flow matching) and Phase 2 (flow matching + contrastive divergence).
# Adapted from the mnist_pcn training script (itself adapted from the original
# CIFAR-10 script), with the CIFAR-10 dataset/architecture restored and the PCN
# (Stage 2 IFT / Stage 3 EP) machinery kept intact.
import os
import sys
import time
import copy
...
from utils_cifar10 import (count_parameters, eqm_c_schedule, generate_and_save,
...
def train_loop(rank, world_size, argv):
    ...
    # -----------------------------------------------------------------------
    # 2) CIFAR-10 dataset with distributed sampler
    # -----------------------------------------------------------------------
    # NOTE: RandomHorizontalFlip restored (unlike MNIST, where flipped digits
    # are invalid) — a mirrored CIFAR-10 photo is still a valid photo.
    ...
    if rank == 0:
        dataset = datasets.CIFAR10(
            ...
            transform=transforms.Compose([
                ...
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # [0,1] → [-1,1]: (x - 0.5) / 0.5
            ])
        )
