...
as PCN prediction functions f_k, one PC state node per UNet block (27 nodes for
the CIFAR-10 config: E=12 encoder blocks, middle, D=12 decoder blocks, out
projection, ViT head), with skip connections expressed as extra DAG parents:
    ...
    node  -2     : out (GN→SiLU→Conv)   parents [prev]         → (B,3,32,32)
...
class ViTHead(nn.Module):
    ...
    def __init__(self, patch_embed, transformer_encoder, final_linear):
        ...
        # prototype layer transformer_nlayers times (8 for CIFAR, 2 for MNIST),
    ...
    def forward(self, x):
        # x = the UNet output IMAGE (B, 3, 32, 32) — i.e. the previous node's state.
        # Conv(k=4,s=4) cuts it into non-overlapping 4x4 patches (32/4=8 per side
        # -> N=64 tokens) and embeds each to width E, + learnable pos-embed.
        tokens = self.patch_embed(x)               # (B, N, E)   e.g. (B, 64, 384)
...
class PCNUNetEnergyModel(PCNEnergyModelBase):
    ...
    def __init__(self, num_channels=128, num_res_blocks=2, channel_mult=(1, 2, 2, 2),
                 attention_resolutions="16", num_heads=4, num_head_channels=64,
                 embed_dim=384, transformer_nheads=4, transformer_nlayers=8,
                 patch_size=4, dim=(3, 32, 32),
        ...
        self.L = len(self.layers)          # E + 1 + D + 2 (27 for CIFAR config)
