...
# Adapted from CIFAR-10 for MNIST FID evaluation.
#
# NOTE: FID uses InceptionV3, which expects 3-channel 299×299 inputs.
# For MNIST (1×28×28), images are resized and channel-replicated
# internally by torchmetrics. FID scores may not be directly
# meaningful or comparable to CIFAR-10 FID scores.
#
# Usage example:
...
##############################################################################
# 1) MNIST Data (single GPU)
##############################################################################

def get_mnist_train_loader(batch_size, num_workers, root=None):
    """Returns a standard DataLoader for MNIST train set."""
    if root is None:
        root = os.environ.get("MNIST_PATH", "./data")
...
def main(argv):
    ...
    logging.get_absl_handler().use_absl_log_file(program_name="fid_mnist", log_dir=savedir)
    ...
    net_model = EBViTModelWrapper(
        ...
        # ViT-specific:
        patch_size=7,  # NOTE: not specified in paper for MNIST. Unsure.
    ...
    # ------------------------------------------------------------
    # MNIST paper: τs = 2.0, so sample out to t=2.0 plus a few extra for sweep
    times_to_sample = [1.0, 1.25, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0]
    ...
    # NOTE: FID scores for MNIST may not be directly meaningful — see file header.
    ...
    for _ in tqdm(range(n_batches), desc="Generating Fake Data"):
        ...
        # Start from standard normal in [B, 1, 28, 28]
        x = torch.randn(curr_bsz, 1, 28, 28, device=device)
